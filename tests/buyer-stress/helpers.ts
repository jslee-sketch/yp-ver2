...
// ── 환불 ───────────────────────────────────────────────
export async function refundReservation(page: Page, resId: number, reason: string = '스트레스 테스트 환불') {
...
export async function setupAll(page: Page) {
  ...
  // 2. Buyer — fresh unique
  await setToken(page, '');
  const buyerEmail = `stressbuyer_${TS}@test.com`;
  const buyerReg = await socialRegister(page, {
    socialId: `buyer_stress_${TS}`, role: 'buyer',
    nickname: `스트레스바이어${TS % 10000}`, email: buyerEmail,
  });
  ...
  if (state.buyerToken) {
    ...
    console.log(`[Setup] buyer reg failed: ${JSON.stringify(buyerReg)}`);
  }
  console.log(`[Setup] buyer: ${state.buyerId}`);

  // 3. Seller — fresh unique
  await setToken(page, '');
  const sellerEmail = `stressseller_b_${TS}@test.com`;
  const sellerReg = await socialRegister(page, {
    socialId: `seller_b_stress_${TS}`, role: 'seller',
    nickname: `바이어테스트셀러${TS % 10000}`, email: sellerEmail,
    business_name: '스트레스상점B', business_number: `${TS}3`,
  });
  ...
  const s2Reg = await socialRegister(page, {
    socialId: `seller_b2_${TS}`, role: 'seller',
    nickname: `바이어테스트셀러2_${TS % 10000}`, email: `stressseller_b2_${TS}@test.com`,
    business_name: '스트레스상점B2', business_number: `${TS}4`,
  });
  ...
  for (let i = 0; i < state.dealIds.length; i++) {
    ...
    else console.log(`[Setup] offer fail: ${JSON.stringify(r)}`);
  }
  console.log(`[Setup] offers: ${JSON.stringify(state.offerIds)}`);

  // 7. Seller2 creates competing offers (lower prices)
  await setToken(page, state.seller2Token);
  const prices2 = [940000, 235000, 375000, 570000, 17000];
  for (let i = 0; i < state.dealIds.length; i++) {
    const r = await createOffer(page, state.dealIds[i], state.seller2Id, prices2[i]);
    if (r.data?.id) state.offerIds.push(r.data.id);
  }

  // 8. Buyer creates & pays 15 reservations
  await setToken(page, state.buyerToken);
