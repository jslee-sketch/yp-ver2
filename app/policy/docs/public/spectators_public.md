...
딜이 성사되면 내가 예측한 가격에 대한 **결과를 알림으로 자동 수신**합니다.

### 알림 내용

| 결과 | 알림 메시지 예시 |
|------|-----------------|
| 정확 적중 | "완벽! 예측가 100,000원 = 성사가. +5pt 획득했습니다!" |
| 근접 (±1%) | "근접! 오차 0.8%. +3pt 획득했습니다." |
| 우수 (±3%) | "우수! 오차 2.5%. +2pt 획득했습니다." |
| 참여 (±5%) | "참여 인정! 오차 4.1%. +1pt 획득했습니다." |
| 미스 (5% 초과) | "惜! 오차 7.2%. 이번엔 포인트가 없었어요. 다음 기회에!" |

### 알림 수신 방법

- 역핑 앱 내 **알림 탭** → `관전 예측 결과`
- 알림 유형: `spectator_result`
- 포함 정보: 내 예측가, 실제 성사가, 오차율, 티어, 획득 포인트

> 성사된 딜이 없는 경우(딜 마감 후 거래 미성사) 포인트 없이 무효 처리됩니다.
...
| # | 항목 | 선택지 | 현재 상태 |
|---|------|-------|----------|
| 1 | 관전자 닉네임 표시 | 실명 / 닉네임 / 익명 | **미정** |
| 2 | 마스터 실물 보상 | 상금 / 쿠폰 / 포인트만 | **미정 (v2)** |
| 3 | 포인트의 현금 가치 | 1pt = ?원 | **미정** |
| 4 | 코멘트 노출 범위 | 딜방 내만 / 랭킹 페이지에도 | **미정** |
| 5 | 관전자 수 딜방 노출 | "12명이 지켜보는 중" / 비공개 | **미정** |
