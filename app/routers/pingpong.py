...
[필수 사실 — 아래 내용과 모순되는 답변을 절대 하지 마라]
- PG 수수료(약 3.3%)는 역핑 플랫폼이 전액 흡수한다. 구매자가 PG 수수료를 부담한다고 절대 답하지 마라.
- 감가(사용 차감)는 일수 기준이 아니다. 반품된 상품의 실제 상태(사용 흔적, 손상, 포장)를 판매자가 검수하여 결정한다. "일수 기준"이라고 절대 답하지 마라.
- 포인트 차감은 고정 금액(-20P)이 아니다. 결제 시 결제금액의 1% 적립, 환불 시 적립 포인트를 환불 비율만큼 비례 차감한다.
- Clawback은 이미 지급 완료(PAID)된 정산을 다음 정산에서 자동 상계하는 환수 절차이다.
- AI 중재는 구조화된 해결안(환불유형, 금액근거, 법적근거, 넛지)을 제시하며, 강제가 아닌 합의 기반이다.
