...
_FAQ_DIRECT_MAP: Dict[str, Any] = {
    ...
    # ---- 필수 사실 교정 (잘못된 답변 방지) ----
    # Q004: 무료배송 환불 시 배송비
    "무료배송 환불": "무료배송이라도 구매자 변심(귀책) 환불이면 왕복배송비(원래배송비+반품배송비)가 환불 금액에서 차감됩니다. 무료배송은 판매자가 부담하는 것이지 배송비가 0원이 아닙니다. 셀러 귀책(불량/오배송)이면 배송비 차감 없습니다.",
    "무료배송 변심": "무료배송이라도 구매자 변심(귀책) 환불이면 왕복배송비(원래배송비+반품배송비)가 차감됩니다. 셀러 귀책이면 배송비 차감 없습니다.",
    "무료배송 배송비": "무료배송 상품이라도 구매자 귀책(변심) 환불 시에는 왕복배송비가 차감됩니다. 무료배송은 판매자가 배송비를 부담하는 것이며, 변심 환불 시 판매자가 부담했던 최초 배송비 + 반품 배송비를 환불 금액에서 차감합니다.",
    # Q010: 개봉 감가
    "개봉 감가": "개봉만으로는 감가가 적용되지 않습니다! 감가는 사용 흔적이 있어야 적용됩니다. 판매자가 반품 상품을 검수(PASS/PARTIAL/FAIL)하여 실제 사용 흔적/손상이 확인된 경우에만 감가율을 적용합니다.",
    "개봉만": "개봉만으로는 감가 불가합니다. 감가는 실제 사용 흔적이 있어야 적용됩니다. 검수 기반으로 판매자가 PASS/PARTIAL/FAIL을 판정하며, 단순 개봉은 PASS(감가 없음)입니다.",
    # Q064: 수수료 환급
    "수수료 환급": "역핑 특혜: 환불 시 플랫폼 수수료도 판매자에게 환급합니다! 판매자가 수수료를 손해보지 않도록 보호하는 역핑만의 정책입니다. PG 수수료는 별도로 역핑이 흡수합니다.",
    ...
    "소액 보상": "보상 금액이 결제금액의 10% 이하이면 반품 없이 즉시 보상됩니다. 상품은 그대로 보유하시면 됩니다. 배송비 차감도 없습니다.",
    "반품 면제": "보상 금액이 결제금액의 10% 이하이면 반품 절차 없이 즉시 보상됩니다. 예: 10만원 결제, 8천원 보상 → 반품 불필요.",
    "반품 안 해도": "소액 보상(결제금액의 10% 이하)인 경우 반품 없이 즉시 보상됩니다. 그 이상이면 반품이 필요합니다.",
    ...
    # ---- WARN 10건 커버 FAQ ----
    "바로 돈 돌아": "환불 요청 후 판매자 확인(2영업일) → 반품(필요시) → 검수 → PG 환불(카드 3~5영업일) 순서로 진행됩니다. 즉시 환불은 아닙니다.",
    "바로 돈": "환불은 즉시가 아닙니다. 판매자 확인(2영업일) → 검수 → PG 환불(3~5영업일). 총 3~10영업일 소요됩니다.",
    "환불 무시": "판매자가 2영업일 내 미응답 시 자동 승인됩니다. 구매자 보호를 위한 역핑 정책입니다.",
    "무시하면": "판매자가 2영업일 내 미응답 시 환불이 자동 승인됩니다. 구매자가 불이익을 받지 않도록 보호합니다.",
    "환불 포인트": "포인트는 환불 금액에 비례하여 차감됩니다. 고정 금액(-20P)이 아닌, 환불 비율만큼 비례 차감입니다.",
    "환불 시뮬레이터 어디": "구매자: /buyer/refund-simulator, 판매자: /seller/refund-simulator, 관리자: /admin/refund-simulator 에서 환불 예상 금액을 미리 계산할 수 있습니다.",
    "교환 상품 불량": "교환받은 상품이 불량이면 새로운 분쟁을 신청하거나 환불로 전환할 수 있습니다.",
    "교환 안 보내면": "판매자가 교환 상품을 3영업일 초과 미발송 시 관리자가 개입합니다. 환불로 전환될 수 있습니다.",
    "반품 기한 넘기": "반품 기한(7일) 초과 시 환불이 자동 취소되고 정산 보류가 해제됩니다. 7일 이내 반품 발송 필수!",
    "반품 기한": "반품은 수령 후 7영업일 이내에 발송해야 합니다. 기한 초과 시 환불이 자동 취소됩니다.",
    "수령 안 했다고": "반품 운송장 번호로 배송 추적이 가능합니다. 배송 완료 기록이 증거로 활용됩니다.",
    "보상금 정산": "보상 금액만큼 판매자 정산에서 차감됩니다. 이미 지급된 경우 Clawback(환수)으로 처리됩니다.",
    "수수료 돌려": "역핑 특혜: 환불 시 플랫폼 수수료도 판매자에게 환급합니다! PG 수수료는 역핑이 흡수합니다.",
    # W1: Q052 - AI 금액 마음에 안 들면
    "AI 금액 마음에 안": "AI 중재안이 마음에 안 들면 거절할 수 있습니다. 거절하면 Round 2로 진행되어 재반론 기회가 주어집니다.",
    "AI 중재 거절": "AI 중재안이 마음에 안 들면 거절할 수 있습니다. 거절하면 Round 2로 진행되어 재반론 기회가 주어집니다. Round 2에서도 미합의 시 법적 안내로 이어집니다.",
    # W2: Q073 - 시뮬레이터에서 정산 영향
    "시뮬레이터 정산": "환불 시뮬레이터의 판매자 모드에서 환불이 정산에 미치는 영향을 미리 확인할 수 있습니다. 경로: /seller/refund-simulator",
    "정산 영향": "환불 시뮬레이터의 판매자 모드에서 환불이 정산에 미치는 영향(차감액, Clawback 여부 등)을 미리 확인할 수 있습니다.",
    # W3: Q081 - 관리자가 직접 환불
    "관리자 직접 환불": "ADMIN_PENDING 상태인 건은 관리자가 수동으로 처리할 수 있습니다. 관리자 패널의 환불 관리(/admin/refunds)에서 승인/거절합니다.",
    "판매자 탈퇴": "미처리 환불/분쟁 건이 있으면 판매자 탈퇴가 불가합니다. 정산 보류가 유지됩니다.",
    "판매자가 탈퇴": "미처리 환불/분쟁 건이 있으면 판매자 탈퇴가 불가합니다. 정산 보류가 유지되어 구매자를 보호합니다.",
    "분쟁 교환": "네! 분쟁 제안 시 해결 유형에서 '교환'을 선택할 수 있습니다. 전액환불, 부분환불, 교환, 보상금, 조치불필요 중 선택 가능합니다.",
    "분쟁에서 교환": "네! 분쟁 제안 유형에서 '교환'을 선택할 수 있습니다. 교환 불가가 아닙니다.",
    "분쟁 제안 텍스트": "분쟁 제안은 자유 텍스트가 아닌 구조화 폼으로 입력합니다. 유형 선택(전액환불/부분환불/교환/보상금) + 금액(정액 또는 정률%) + 배송비 부담(내가/상대방/각자) + 반품 여부를 구조화된 폼으로 입력합니다.",
    "분쟁 제안 방식": "분쟁 제안은 구조화 폼입니다. 1)해결 유형 선택, 2)금액(정액/정률), 3)배송비 부담, 4)반품 여부, 5)사유 설명. 이 데이터가 AI 중재와 자동 정산에 반영됩니다.",
    "관리자 환불": "네, 관리자는 환불을 직접 처리할 수 있습니다. /admin/refunds에서 환불 요청 목록을 관리하고, 환불 시뮬레이터(/admin/refund-simulator)로 금액을 사전 확인할 수 있습니다.",
    ...
    # ---- 중재 결렬 후속 처리 ----
    "중재 결렬": "중재가 합의에 이르지 못하면 14일의 유예기간이 주어집니다. 이 기간 동안 상대방과 직접 합의하거나, 한국소비자원(☎1372) 또는 관할 법원 소액사건심판에 신청하실 수 있습니다. 14일 내 추가 조치가 없으면 관리자가 AI 중재안 기준으로 최종 판정합니다.",
    "결렬되면": "중재가 합의에 이르지 못하면 14일의 유예기간이 주어집니다. 직접 합의, 한국소비자원(☎1372) 신청, 소액사건심판 중 선택할 수 있습니다. 14일 내 추가 조치가 없으면 관리자가 AI 중재안 기준으로 최종 판정합니다.",
    "직접 합의": "중재 상세 페이지에서 [직접 합의안 등록] 버튼을 클릭하세요. 보상 금액과 처리 방식을 입력하면 상대방에게 알림이 갑니다. 상대방이 7일 내 수락하면 즉시 환불/정산이 처리됩니다.",
    "직접 합의 어떻게": "중재 상세 페이지에서 [직접 합의안 등록] 버튼을 클릭하세요. 보상 금액(정액/정율)과 처리 방식(환불/부분환불/교환)을 입력하면 상대방에게 알림이 갑니다. 상대방이 7일 내 수락하면 즉시 환불/정산이 자동 처리됩니다.",
    "소비자원": "한국소비자원에 피해구제를 신청하려면 ☎1372 또는 www.kca.go.kr을 이용하세요. 신청 후 중재 상세 페이지에서 [외부기관 신청 등록] 버튼으로 등록하시면 정산 보류가 최대 90일까지 자동 연장됩니다.",
    "소비자원 신청": "소비자원에 신청 후 중재 상세 페이지에서 [외부기관 신청 등록]을 눌러주세요. 등록하면 정산 보류가 최대 90일까지 자동 연장됩니다. 외부기관의 결정이 나오면 관리자가 결과를 반영합니다.",
    "소액사건심판": "소송가액 3,000만원 이하의 분쟁을 관할 법원에서 간이하게 처리하는 제도입니다. 1회 변론으로 판결이 나며, 법적 강제력이 있습니다. 관할 법원에 직접 신청하세요.",
    "관리자 최종 판정": "14일 유예기간이 지나도 양측이 아무 조치를 취하지 않으면, 관리자가 AI 중재안을 기준으로 최종 판정을 내립니다. 이 판정에 따라 환불/정산이 자동 처리됩니다.",
    "관리자 판정": "14일 유예기간이 지나도 양측이 아무 조치를 취하지 않으면, 관리자가 AI 중재안을 기준으로 최종 판정을 내립니다. 이 판정에 따라 환불/정산이 자동 처리됩니다.",
    "정산 보류": "중재가 접수되면 해당 주문의 정산만 보류됩니다. 다른 주문의 정산은 영향받지 않습니다. 중재가 합의되거나 관리자가 판정하면 보류가 해제되고 정산이 재계산됩니다.",
    "중재 신청 조건": "중재를 신청하려면 ① 결제 완료된 주문이어야 하고, ② 해당 주문에 거절된 교환/반품 요청이 있어야 합니다. 거절 이력이 없으면 먼저 교환/반품을 신청해주세요.",
    "보상금": "중재 신청 시 보상금은 필수 입력입니다. 정액(예: 50,000원) 또는 정율(예: 10%)로 입력할 수 있습니다. 상대방과 AI도 각각 제안을 하며, 3자 제안 중 하나를 선택하게 됩니다.",
    # ---- 공통 (새 기능) ----
    ...
    "뭘 도와": {
        "BUYER": "딜 생성(사진/음성/텍스트), 시장가 조회, 가격 챌린지, 배송 추적, 환불 안내, 중재 신청 등을 도와드려요! 😊",
