...
export default function AdminRefundsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const r = await apiClient.get(API.ADMIN.RESERVATIONS, { params: { refund: true, limit: 200 } });
          ...
          <tbody>
            {items.filter(r => { const q = search.toLowerCase(); return !q || [String(r.id), r.buyer_name, r.seller_name].some(v => v && String(v).toLowerCase().includes(q)); }).map(r => (
              <tr key={r.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: '10px 8px' }}><span style={{ color: C.red, fontWeight: 600 }}>{r.status}</span></td>
                <td style={{ padding: '10px 8px', color: C.textSec }}>{r.refunded_at ? new Date(r.refunded_at).toLocaleDateString('ko-KR') : '-'}</td>
              </tr>
            ))}
            {!items.length && <tr><td colSpan={7} style={{ padding: 24, textAlign: 'center', color: C.textSec }}>환불 없음</td></tr>}
          </tbody>
        </table>
      </div>
