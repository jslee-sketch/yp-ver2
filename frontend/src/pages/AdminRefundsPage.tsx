...
export default function AdminRefundsPage() {
  ...
  const load = async () => {
    try {
      const params: any = { refund: true, limit: 300 };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="R-#/구매자/판매자/품목명 검색" style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: `1px solid ${C.border}`, background: C.card, color: C.text, fontSize: 13 }} />
      </div>
        ...
        <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 800 }}>
            ...
            <tbody>
              ...
              {!items.length && <tr><td colSpan={11} style={{ padding: 24, textAlign: 'center', color: C.textSec }}>환불 없음</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
