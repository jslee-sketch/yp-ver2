            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                  <td style={{ padding: '10px 8px', color: '#4ade80', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => window.open(`/mypage?user_id=${s.user_id || s.id}`, '_blank')}>S-{s.id}</td>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setModal(null)}>
          <div style={{ background: C.card, borderRadius: 16, padding: 24, minWidth: 400, maxWidth: 520, border: `1px solid ${C.border}`, maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            {Object.entries({ 상호명: modal.business_name, 이메일: modal.email, 사업자번호: modal.business_number, 레벨: modal.level, 대표자: modal.representative_name, 전화: modal.phone, 포인트: modal.points, 액추에이터: modal.actuator_id ? `ACT-${String(modal.actuator_id).padStart(5,'0')}` : '-', 승인일: modal.verified_at }).map(([k, v]) => (
